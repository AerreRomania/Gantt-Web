...
function initializeGanttChartTemplates(settings, theme) {
    ...
    settings.styleDefinitionTemplateForTheme = function (ganttChartView) {
    ...
    if (!settings.styleDefinitionTemplate)
        settings.styleDefinitionTemplate = settings.styleDefinitionTemplateForTheme;
    settings.standardTaskTemplateForTheme = function (item) {
    ...
    if (!settings.standardTaskTemplate)
        settings.standardTaskTemplate = settings.standardTaskTemplateForTheme;
    settings.summaryTaskTemplateForTheme = function (item) {
        var ganttChartView = item.ganttChartView;
        var settings = ganttChartView.settings;
        var items = ganttChartView.items;
        var document = ganttChartView.ownerDocument;
        ...
        var bar = document.createElementNS(svgns, 'rect');
        bar.setAttribute('x', itemLeft);
        bar.setAttribute('y', settings.barMargin * 1.38);
        bar.setAttribute('width', Math.max(0, itemRight - itemLeft - 1));
        bar.setAttribute('height', height * 1.38);
        ...
        if (!item.isExpanded) {
            ...
            line.setAttribute('y1', settings.barMargin * 1.38 + height * 1.38 + 2.5);
            line.setAttribute('x2', itemRight - 1);
            line.setAttribute('y2', settings.barMargin * 1.38 + height * 1.38 + 2.5);
        ...
        var y = settings.barMargin * 1.38 - 0.25, h = height * 1.38 + 1.5, tw = settings.barHeight * 1.38 * 3 / 4, teh = settings.barHeight * 1.38 / 4;
        x = itemLeft - 1 - settings.barHeight * 1.38 / 3;
        ...
        x = itemRight + settings.barHeight * 1.38 / 3;
        ...
        if (!settings.isReadOnly && !settings.isChartReadOnly && (typeof item.isReadOnly === undefinedType || !item.isReadOnly) && (typeof item.isBarReadOnly === undefinedType || !item.isBarReadOnly)) {
            if (settings.areTaskDependenciesVisible && !settings.areTaskPredecessorsReadOnly && !item.isPart) {
                ...
                if (typeof settings.allowCreatingStartDependencies === undefinedType || settings.allowCreatingStartDependencies) {
                    ...
                    startDependencyThumb.setAttribute('cy', settings.barMargin * 1.38 + settings.barHeight * 1.38 / 2);
                    startDependencyThumb.setAttribute('r', settings.barHeight * 1.38 / 4);
                    startDependencyThumb.setAttribute('style', 'fill: White; fill-opacity: 0; cursor: pointer');
                    group.appendChild(startDependencyThumb);
                }
                var dependencyThumb = document.createElementNS(svgns, 'circle');
                dependencyThumb.setAttribute('cx', itemRight - 0.5);
                dependencyThumb.setAttribute('cy', settings.barMargin * 1.38 + settings.barHeight * 1.38 / 2);
                dependencyThumb.setAttribute('r', 2.5);
                dependencyThumb.setAttribute('style', 'fill: White; fill-opacity: 0; cursor: pointer');
                group.appendChild(dependencyThumb);
                ganttChartView.initializeDependencyDraggingThumbs(dependencyThumb, startDependencyThumb, group, item, settings.barMargin * 1.38 + settings.barHeight * 1.38 / 2, itemRight - 1.5, itemLeft);
            }
        }
        return group;
    };
    if (!settings.summaryTaskTemplate)
        settings.summaryTaskTemplate = settings.summaryTaskTemplateForTheme;
    settings.milestoneTaskTemplateForTheme = function (item) {
        var ganttChartView = item.ganttChartView;
        var settings = ganttChartView.settings;
        var items = ganttChartView.items;
        var document = ganttChartView.ownerDocument;
        ...
        if (settings.isBaselineVisible && typeof item.baselineStart !== undefinedType) {
            ...
            var xb = itemBaselineLeft, yb = settings.barMargin * 1.38 - 1, hb = settings.barHeight * 1.38 + 1;
        ...
        var x = itemLeft - 1, y = settings.barMargin * 1.38, h = settings.barHeight * 1.38 + 1;
        ...
        if (!settings.isReadOnly && !settings.isChartReadOnly && (typeof item.isReadOnly === undefinedType || !item.isReadOnly) && (typeof item.isBarReadOnly === undefinedType || !item.isBarReadOnly)) {
            ...
            if (settings.areTaskDependenciesVisible && !settings.areTaskPredecessorsReadOnly && !item.isPart) {
                ...
                dependencyThumb.setAttribute('cy', settings.barMargin * 1.38 + settings.barHeight * 1.38 / 2);
                dependencyThumb.setAttribute('r', settings.barHeight * 1.38 / 4);
                dependencyThumb.setAttribute('style', 'fill: White; fill-opacity: 0; cursor: pointer');
                group.appendChild(dependencyThumb);
                ganttChartView.initializeDependencyDraggingThumbs(dependencyThumb, null, group, item, settings.barMargin * 1.38 + settings.barHeight * 1.38 / 2, x, x);
            }
        }
        return group;
    };
    if (!settings.milestoneTaskTemplate)
        settings.milestoneTaskTemplate = settings.milestoneTaskTemplateForTheme;
    settings.dependencyLineTemplateForTheme = function (item, predecessorItem) {
        var ganttChartView = item.ganttChartView;
        var settings = ganttChartView.settings;
        var items = ganttChartView.items;
        var document = ganttChartView.ownerDocument;
            ...
            if (y == y1 && (typeof predecessorItem.dependencyType === undefinedType || predecessorItem.dependencyType == 'FinishStart' || predecessorItem.dependencyType == 'FS' || predecessorItem.dependencyType == 'StartFinish' || predecessorItem.dependencyType == 'SF')) {
                ...
                if (typeof predecessorItem.dependencyType !== undefinedType && (predecessorItem.dependencyType == 'StartStart' || predecessorItem.dependencyType == 'SS' || predecessorItem.dependencyType == 'StartFinish' || predecessorItem.dependencyType == 'SF')) {
                    ...
                    if ((predecessorItem.item.hasChildren && (typeof predecessorItem.item.isSummaryEnabled === undefinedType || predecessorItem.item.isSummaryEnabled)))
                        x -= settings.barMargin * 1.38 * 5 / 8 + 0.25;
                ...
                if (typeof predecessorItem.dependencyType !== undefinedType && (predecessorItem.dependencyType == 'FinishFinish' || predecessorItem.dependencyType == 'FF' || predecessorItem.dependencyType == 'StartFinish' || predecessorItem.dependencyType == 'SF')) {
                    ...
                    if (typeof predecessorItem.dependencyType !== undefinedType && (predecessorItem.dependencyType == 'FinishFinish' || predecessorItem.dependencyType == 'FF')) {
                        ...
                        if (item.hasChildren && (typeof item.isSummaryEnabled === undefinedType || item.isSummaryEnabled))
                            x += settings.barMargin * 1.38 * 5 / 8 + 1;
                ...
                if (horizontal)
                    ...
                    y = settings.barMargin * 1.38 - (!item.isMilestone && !item.hasChildren ? barMargin : 1) - arrowSpace;
                else
                    y = h - settings.barMargin * 1.38 + (!item.isMilestone && !item.hasChildren ? barMargin : 1) + arrowSpace;
                if (horizontal) {
                    if (x2 > x - arrowSpace)
    ...
    if (!settings.dependencyLineTemplate)
        settings.dependencyLineTemplate = settings.dependencyLineTemplateForTheme;
    settings.assignmentsTemplateForTheme = function (item) {
        var ganttChartView = item.ganttChartView;
        var settings = ganttChartView.settings;
        var document = ganttChartView.ownerDocument;
    ...
    if (!settings.assignmentsTemplate)
        settings.assignmentsTemplate = settings.assignmentsTemplateForTheme;
    settings.itemTemplateForTheme = function (item) {
        var ganttChartView = item.ganttChartView;
        var document = item.ganttChartView.ownerDocument;
    ...
    if (!settings.itemTemplate)
        settings.itemTemplate = settings.itemTemplateForTheme;
    settings.predecessorItemTemplateForTheme = function (item, predecessorItem) {
        var document = item.ganttChartView.ownerDocument;
        var toolTip = document.createElementNS(svgns, 'title');
